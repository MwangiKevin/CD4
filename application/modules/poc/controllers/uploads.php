...
class uploads extends MY_Controller {
	...
	public function server_upload(){
		...
		$root_folder = "C:/FTP/PIMA-Export";
		$uploaded_folder = "C:/FTP/PIMA-Uploaded";
		...
		if ($handle = opendir($root_folder)) {
		    while (false !== ($entry = readdir($handle))) {
		        ...
		        if(substr($entry, -4)==".csv"&& substr($entry, -13)!="(Unknown).csv"){
		        	//echo "<br/>".$entry;
		        	if($this-> server_upload_commit(realpath($root_folder."/".$entry))){
		        		...
		        		// $files_to_move[$i+1]["source"] 		= 	$root_folder."/".$entry.".md5";
					    // $files_to_move[$i+1]["destination"]	=	$uploaded_folder."/".$entry.".md5";
	...
	public function server_upload_commit($file){		
		...
		if(sizeof($data) > 0) {
			...
			if(sizeof($facility_pima_id_res)>0){
				...
				$upload_success = false;
				$this->device_not_recognized($serial_num,1);
			}
		}else{
	...
	public function upload_commit(){
		...
		if(sizeof($data) > 0) {
			...
			if(sizeof($facility_pima_id_res)>0){
				...
				$facility_pima_id=	0;
				$view_data['message']= "<div class='error'>This device is not recognized by the system</div>";				
				$this->device_not_recognized($serial_num,$this->get_current_user_id());
			}
		}else{
			$view_data['message']= "<div class='notice'>No data Uploaded! <br/>It seems all of this data has already been uploaded or there is no data in the file.</div>";
		}
