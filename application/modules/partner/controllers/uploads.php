...
class uploads extends MY_Controller {
	...
	public function index(){
		...
		$data['menus']	= 	$this->partner_model->menus(2);
		$this -> template($data);
	}
	...
	public function upload_commit(){
		...
		$view_data['content_view'] 		= 	"partner/uploads_view";
		$view_data['title'] 			= 	"Uploads";
		$view_data['sidebar']			= 	"partner/sidebar_view";
		$view_data['posted']			=	0 ;
		...
		//print_r($upload_data);
		//print_r($sheet_data);
		$view_data	=	array_merge($view_data,$this->load_libraries(array("dataTables", "partner_uploads")));
		
		$this->load->model('partner_model');

		$view_data['menus']	= 	$this->partner_model->menus(2);
